...
void  OSStatInit (void)
{
    ...
    OS_EXIT_CRITICAL();
    OSTimeDly(OS_TICKS_PER_SEC / 10);            /* Determine MAX. idle counter value for 1/10 second  */ //初始化，在任务未跑起来的时候都是IDLE，这时候计算出OSIdleCtrMax
    OS_ENTER_CRITICAL();
            ...
            if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
                if (--ptcb->OSTCBDly == 0) {               /* Decrement nbr of ticks to end of delay       */
                    ...
                    if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) != OS_STAT_RDY) {
                        ...
                        ptcb->OSTCBPendTO  = OS_TRUE;                          /* Indicate PEND timeout    */					//超时未收到
                    } else {
                        ptcb->OSTCBPendTO  = OS_FALSE;																			//接收到信号量
                    }
...
void  OS_TaskIdle (void *p_arg)
{
    ...
    for (;;) {
		...
		if((OSIntNesting==0)&&(StopModeLock==0)&&(BSP_UART_TxState(2)==0)&&(BSP_UART_TxState(1)==0)&&(BSP_UART_TxState(3)==0))   
			//1.StopModeLock：手动添加的变量，在需要保护的地方禁入STOP，避免出问题（包括等待邮箱，存数过程等）；2.串口发送过程，中断函数处理过程禁入STOP
		{
			OSSchedLock();
...
//		MCU_INSTOP();			//睡眠模式（内核休眠，IO口及外设不动），可被每个systick中断唤醒    //弃用
		PWR_EnterSLEEPMode(PWR_SLEEPNow,PWR_SLEEPEntry_WFI);    //sleep mode	这玩意会被任何中断唤醒，包括systick
		}
    }
}
/*$PAGE*/
...
void  OS_TaskStat (void *p_arg)
{
    ...
    max = OSIdleCtrMax / 100L;                    //100等分
    for (;;) {
        OS_ENTER_CRITICAL();
        OSIdleCtrRun = OSIdleCtr                ;/* Obtain the of the idle counter for the past second */ //OSIdleCtr在IDLE任务中++，在这里清零
        run          = OSIdleCtr;
