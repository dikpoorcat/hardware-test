...
*                    the downcounter.
*                    This parameter value must be lower than 0x80.( WindowValue<=0x7F )
* Output         : None
* Return         : None
*******************************************************************************/
...
void WWDG_Enable(u8 Counter)
{
  ...
  WWDG->CR = CR_WDGA_Set | Counter;          //zzs note, 设置计数器初值的同时，就将使能位置1
	/*
	while(1)
		{
	    ReadDogCnt=WWDG->CR;   //zzs note, 别天真了，读不到想象当中的值的
			if(ReadDogCnt<=0x45)
			{
			  ReadDogCnt=0;
			}
